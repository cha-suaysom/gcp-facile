...
class Sample(object):
    ...
    def setvalues(name, base, region):
        ...
        if region != '':
            self.X = pd.read_pickle('%s/%s_%s.pkl'%(base,'X', region))[:Nrhs]
            self.Y = pd.read_pickle('%s/%s_%s.pkl'%(base,'Y', region))[:Nrhs]
            self.kin = pd.read_pickle('%s/%s_%s.pkl'%(base,'X', region))[:Nrhs][['PU','ieta','iphi','pt']]
        else:
